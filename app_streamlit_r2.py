...
with col1:
    folder_path = st.text_input("Insérez le chemin du dossier")

if folder_path:
    with col2:
        filenames = os.listdir(folder_path)
        selected_filename = st.selectbox('PRE | POST', filenames)

    with col3:
        file = sorted(os.listdir(folder_path+'/'+selected_filename))
        selected_file = st.selectbox("Sélectionnez votre fichier sprint", file)

    filename = os.path.join(folder_path, selected_filename, selected_file)
    filename_2 = os.path.join(folder_path, selected_filename, selected_file.replace("1","2"))

    st.markdown(f'<h2 style="background-color:#b1cbd8;border-radius:14px;text-align:center;">{f"ANALYSE : {selected_filename} | {selected_file[:-6]}"}</h2>', unsafe_allow_html=True)
    ...
    # DATAFRAME FINALE 
    new_df = pd.DataFrame(columns = ["NOM","TEST","A_1","B_1","R2_1","A_2","B_2","R2_2","A_3","B_3","R2_3","Pmax_1","Pmax_2","Pmax_3"])
